...
# CSCI211 Stormtracker Station Linked List Exercise
...
def _partition(start,stop,attr):
    ...
    pivot = stop.get_data()
    log.debug(f"Partitioner: Start {start.get_data()}, Stop: {stop.get_data()} Pivot Data: {pivot}. Attr: {attr}")
    ...
    while j != stop:
        ...
        if (getattr(j.get_data(),attr) <= getattr(pivot,attr)):
            log.info(f"Iteration is <=  Pivot: {getattr(pivot,attr)}.")
...
def _linked_quick_sort(start,stop,attr):
    if (start != False) and (start != stop)  and (stop != False) and (stop.next != start):
