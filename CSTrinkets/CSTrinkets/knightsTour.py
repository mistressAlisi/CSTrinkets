...
def get_moves(start,matrix,size,depth=0):
    moves = list()
    moves.append(KnightPos(matrix,start.x + 1, start.y + 2))
    moves.append(KnightPos(matrix,start.x + 1, start.y - 2))
    moves.append(KnightPos(matrix,start.x - 1, start.y + 2))
    moves.append(KnightPos(matrix,start.x - 1, start.y - 2))
    moves.append(KnightPos(matrix,start.x + 2, start.y + 1))
    moves.append(KnightPos(matrix,start.x + 2, start.y - 1))
    moves.append(KnightPos(matrix,start.x - 2, start.y + 1))
    moves.append(KnightPos(matrix,start.x - 2, start.y - 1))
    valid_moves = [
...
def run_knights_tour(start, size, the_trie, matrix, current_string="",depth=0,CurrentString="",CurrentLength=0):
    moves_data = get_moves(start, matrix,size,depth)
    moves = moves_data[0]
    depth = moves_data[1]
    ...
    for move in moves:
        ...
        if (len(the_trie.search(candidate)) > 0):
            # IF we have a single candidate left, we're at the end of our journey!:
            candidate_search  = the_trie.search(candidate)
            ...
            candidate = run_knights_tour(move, size, the_trie, matrix, candidate,depth,CurrentString,CurrentLength)
...
def the_grand_tour(the_trie,matrix,size=8):
    count = 0
    ...
    for i in range(size):
        for j in range(size):
            start = KnightPos(matrix,i, j)
            # Don't visit squares that don't hold characters that don't form part our Trie:
            if (len(the_trie.search(start.char())) > 0):
                current_string = start.char()
                dbg_output += "*** Knight is Touring from Square: {},{}:{} ***\n".format(i,j,start.char())
                tour_data =  run_knights_tour(start, size,the_trie,matrix,current_string,0)
    ...
    return [longest_string,longest_string_len,longest_string_pos,dbg_output]
